...
@section('content')
    <div class="pb-5 text-xl text-gray-700">
        <div>
            <p class="text-3xl font-semibold">Daily Car Lists</p>
        </div>

        <div class="flex justify-between gap-4 mt-4">
            <div>
                <form method="GET" action="{{ route('daily-car-lists.index') }}">
            ...
            <div>
                <a href="{{ route('daily-car-lists.create') }}" type="button"
        ...
        <div id="popupDiv">
            <div>
                <form method="POST" action="{{ route('daily-car-lists.excelExport') }}">
                    @csrf
        ...
        <div class="p-5 text-base bg-white border border-gray-200 rounded-lg shadow width-auto">
            ...
            <table class="table table-bordered table-striped table-hover datatable daily-cars-table w-100">
                <thead>
                    <tr>
                        <th>No</th>
                        <th>Ref No</th>
                ...
                <tbody>
                    @if (!$daily_cars->isEmpty())
                        @php
                            $no = 1;
                        @endphp
                        @foreach ($daily_cars as $key => $daily_car)
                            <tr data-entry-id="{{ $daily_car->id }}">
                                <td>{{ $no }}</td>
                                <td>{{ $daily_car->ref_no }}</td>
                                <td>{{ date('d-M-Y', strtotime($daily_car->date)) }}</td>
                                <td>{{ $daily_car->sr_no }}</td>
                                <td>{{ $daily_car->tour->short_name }}</td>
                                <td>{{ $daily_car->car->car_no }}</td>

                                <td class="flex">
                                    <a class="p-2 action_icon" href=" {{ route('daily-car-lists.edit', $daily_car) }}">
                                    ...
                                    <form id="delete-{{ $daily_car->id }}" class="inline-block"
                                        action="{{ route('daily-car-lists.destroy', $daily_car->id) }}" method="POST">
                                        @method('DELETE')
                                        @csrf

                                        <button class="p-2 action_icon" type="button"
                                            onclick="if (confirm('Are you sure you want to delete this?')) document.getElementById('delete-{{ $daily_car->id }}').submit();">
...
@section('scripts')
<script>
    new DataTable('.daily-cars-table');
