...
@section('content')
    <div class="pb-5 text-xl text-gray-700">
        <div>
            <p class="text-3xl font-semibold">Cargos</p>
        </div>
        ...
        <div class="flex justify-between gap-4 mt-4">
            ...
            <div>
                <a href="{{ route('cargos.create') }}" type="button"
        ...
        <div id="popupDiv">
            <div>
                <form method="POST" action="{{ route('cargos.excelExport') }}">
                    @csrf
        ...
        <section class="bg-slate-50 px-4 py-2 border rounded-lg shadow my-2 text-gray-800">
            <div class="flex items-center justify-between" id="formToggleSection">
                <p class="text-xl font-semibold">Import Data</p>
            ...
            <div id="formSection">
                <form method="POST" action="{{ route('cargos.import_excel') }}" enctype="multipart/form-data">
                    @csrf

                    <div class="grid grid-cols-4 gap-4">
                        <div class="mb-2">
                            <input
                                class="block w-full text-sm px-1 py-1.5 text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
        ...
        <div class="p-5 text-base bg-white border border-gray-200 rounded-lg shadow width-auto">
            ...
            <table class="table table-bordered table-striped table-hover datatable cargos-table w-100">
                <thead>
                    <tr>
                        <th>No</th>
                        <th>Ref No</th>
                ...
                <tbody>
                    @if (!$cargos->isEmpty())
                        ...
                        @foreach ($cargos as $key => $cargo)
                            <tr data-entry-id="{{ $cargo->id }}" class="border-b">
                                <td>{{ $no }}</td>
                                <td>{{ $cargo->ref_no }}</td>
                                <td>{{ date('d-m-Y', strtotime($cargo->date)) }}</td>
                                <td>{{ $cargo->tour->short_name }}</td>
                                <td>{{ $cargo->car->car_no }}</td>
                                <th>{{ $cargo->cargo_no }}</th>
                                {{-- <th>{{ $cargo->fromCity?->mm_name }} <br> {{ $cargo->fromGate?->mm_name }}</th>
                                <th>{{ $cargo->toCity?->mm_name }} <br> {{ $cargo->toGate?->mm_name }}</th> --}}
                                <td>{{ $cargo->sender_name }} <br> {{ $cargo->sender_phone }}</td>
                                <td>{{ $cargo->receiver_name }} <br> {{ $cargo->receiver_phone }}</td>
                                <td>{{ $cargo->item_name }}</td>
                                <td>{{ $cargo->qty }}</td>
                                <td>{{ $cargo->total }}</td>

                                <td class="flex">
                                    <a class="p-2 action_icon" href=" {{ route('cargos.edit', $cargo) }}">
                                    ...
                                    <form id="delete-{{ $cargo->id }}" class="inline-block"
                                        action="{{ route('cargos.destroy', $cargo->id) }}" method="POST">
                                        @method('DELETE')
                                        @csrf

                                        <button class="p-2 action_icon" type="button"
                                            onclick="if (confirm('Are you sure you want to delete this?')) document.getElementById('delete-{{ $cargo->id }}').submit();">
