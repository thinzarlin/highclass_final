...
@section('content')
    <div class="pb-5 text-xl text-gray-700">
        ...
        <div class="flex justify-between gap-4 mt-4">
            <div>
                <form method="GET" action="{{ route('home-car-main-cbs.index') }}">
            ...
            <div>
                {{-- <a href="{{ route('opening-invoice-credits.index') }}" type="button"
                    class="text-white bg-teal-600 hover:bg-teal-700 focus:ring-4 focus:ring-teal-200 font-medium rounded-lg text-sm px-3 py-2.5 me-2 mb-2 dark:bg-teal-500 dark:hover:bg-teal-600 focus:outline-none dark:focus:ring-teal-700">Opening</a>
        ...
        <div id="popupDiv">
            <div>
                <form method="POST" action="{{ route('home-car-main-cbs.excelExport') }}">
                    @csrf
        ...
        <div
            ...
            <table class="table table-bordered table-striped table-hover datatable home-car-main-cbs-table w-100">
                <thead>
                    <tr>
                        <th>No</th>
                        <th>Ref No</th>
                    ...
                    @if (!$cbs->isEmpty())
                        ...
                        @foreach ($cbs as $key => $cb)
                            <tr data-entry-id="{{ $cb->id }}">
                                <td>{{ $no }}</td>
                                <td>{{ $cb->ref_no }}</td>
                                <td>{{ date('d-M-Y', strtotime($cb->main->out_date)) }}</td>
                                <td>{{ date('d-M-Y', strtotime($cb->main->in_date)) }}</td>
                                <td>{{ $cb->main->tour->short_name }}</td>
                                <td>{{ $cb->main->car->car_no }}</td>
                                <td class="text-end">{{ number_format(($cb->net_amount_debit > $cb->net_amount_credit)? $cb->net_amount_debit: - $cb->net_amount_credit) }}</td>

                                <td class="flex">
                                    <a class="p-2 action_icon" href=" {{ route('home-car-main-cbs.edit', $cb) }}">
                                    ...
                                    <form id="delete-{{ $cb->id }}" class="inline-block"
                                        action="{{ route('home-car-main-cbs.destroy', $cb->id) }}" method="POST">
                                        @method('DELETE')
                                        @csrf
...
@section('scripts')
    <script>
        new DataTable('.home-car-main-cbs-table');
