...
@section('content')
    <div class="pb-5 text-xl text-gray-700">
        <div>
            <p class="text-3xl font-semibold">Diesel Entry</p>
        </div>

        <div class="flex justify-between gap-4 mt-4">
            <div>
                <form method="GET" action="{{ route('diesels.index') }}">
                    <div class="flex justify-start gap-4">
                        ...
                        <div class="mb-2">
                            <input
                                ...
                                id="in_date" type="text" placeholder="အဝင်နေ့" name="in_date"
                                value="{{ $in_date ?? '' }}" autocomplete="off" required>
                        </div>
                        {{-- <div class="mb-2">
                        ...
                        <div class="mb-2">
                            <select
                                class="shadow w-full py-1 ps-3 pe-12 leading-[2] bg-white border border-gray-300 text-gray-900 rounded-lg focus:ring-blue-500 focus:border-blue-500 block"
                                id="daily_car_list_id" name="daily_car_list_id">

                                <option value="" selected>Choose one</option>
                                @foreach ($daily_cars as $key => $daily_car)
                                    <option value="{{ $daily_car->id }}"
                                        {{ $daily_car_list_id == $daily_car->id ? 'selected' : '' }}>
            ...
            <div>
                <a href="{{ route('diesels.create') }}" type="button"
        ...
        <div id="popupDiv">
            <div>
                <form method="POST" action="{{ route('diesels.excelExport') }}">
                    @csrf
        ...
        <div class="p-5 text-base bg-white border border-gray-200 rounded-lg shadow width-auto">
            ...
            <table class="table table-bordered table-striped table-hover datatable diesels-table w-100">
                <thead>
                    <tr>
                        <th>No</th>
                        <th>Ref No</th>
                ...
                <tbody>
                    @if (!$diesels->isEmpty())
                        @php
                            $no = 1;
                        @endphp
                        @foreach ($diesels as $key => $diesel)
                            <tr data-entry-id="{{ $diesel->id }}">
                                <td>{{ $no }}</td>
                                <td>{{ $diesel->ref_no }}</td>
                                <td>{{ date('d-m-Y', strtotime($diesel->out_date)) }}</td>
                                {{-- <td>{{ date('d-m-Y', strtotime($diesel->in_date)) }}</td> --}}
                                <td>{{ date('d-m-Y', strtotime($diesel->purchase_date)) }}</td>
                                <td>{{ $diesel->tour->short_name }}</td>
                                <td>{{ $diesel->car->car_no }}</td>
                                <td>{{ $diesel->shop->name }}</td>
                                <td>{{ config('enums.payment_types.' . $diesel->payment_type) }}</td>
                                <td class="text-end">{{ number_format($diesel->liter, 3) }}</td>
                                <td class="text-end">{{ number_format($diesel->amount) }}</td>

                                <td class="flex">
                                    <a class="p-2 action_icon" href=" {{ route('diesels.edit', $diesel) }}">
                                    ...
                                    <form id="delete-{{ $diesel->id }}" class="inline-block"
                                        action="{{ route('diesels.destroy', $diesel->id) }}" method="POST">
                                        @method('DELETE')
                                        @csrf

                                        <button class="p-2 action_icon" type="button"
                                            onclick="if (confirm('Are you sure you want to delete this?')) document.getElementById('delete-{{ $diesel->id }}').submit();">
            ...
            <div class="flex justify-end gap-5">
                ...
                <div class="mb-2">
                    <input
                        ...
                        id="total_liter" type="text"
                        placeholder="Total Liter" value="{{ number_format($total_liters, 3) }}" readonly>
                </div>

                <div class="mb-2">
                    <input
                        class="text-right w-32 shadow py-1 px-2.5 leading-[2] bg-white border border-gray-300 text-gray-900 rounded-lg focus:ring-blue-500 focus:border-blue-500 block"
                        id="total_amount" type="text"
                        placeholder="Total Amount" value="{{ number_format($total_amount) }}" readonly>
                </div>
            </div>
        </div>
    </div>
...
@section('scripts')
    <script>
        ...
        $(document).ready(function() {
            ...
            @if (isset($start_date))
                picker1.setDate(moment('{{ $in_date }}').toDate());
